  ...
  initPromise = import('nostr-login')
    .then(async ({ init }) => {
      ...
      await init({
        ...
        // Request all permissions upfront to match nsec.app enabled permissions
        // This ensures permissions are requested during initial connection, preventing popup loops
        // Event kinds: 0 (metadata), 1 (notes), 3 (follows), 6 (reposts), 7 (reactions),
        //              9735 (zap requests), 10000 (follow sets/mute lists), 10002 (relay list),
        //              22242 (auth), 27235 (HTTP auth), 30000 (structured content), 30023 (long-form)
